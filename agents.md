...
This is a **Next.js (App Router) / React / TypeScript** application interacting directly with a Wahoo KICKR CORE 2 Smart Trainer.
Communication relies heavily on the **Web Bluetooth API** (`navigator.bluetooth`), specifically the **Bluetooth FTMS (Fitness Machine Service)** protocol.
...
1. **Bluetooth Constraints:** 
   ...
   - Disconnect handling is crucial (`gattserverdisconnected` events).
2. **FTMS Specifics:**
...
- **Chart Visualization:** Use a charting library (like Recharts, which is compatible with shadcn/ui) to plot Power/Cadence data in real time.
