...
7. **Workout Builder and Post-Ride Summary LLM Lanes:**
   ...
   - Configure `WORKOUT_BUILDER_API_KEY` / `WORKOUT_BUILDER_MODEL` for this lane. It falls back through `RIDE_SUMMARY_API_KEY`, `LIVE_COACH_API_KEY`, `WORKOUT_IMAGE_EXTRACTOR_API_KEY`, and `AI_GATEWAY_API_KEY`.
   ...
   - Configure `RIDE_SUMMARY_API_KEY` / `RIDE_SUMMARY_MODEL` for this lane. If no key is configured, saving still succeeds with `llmSummaryStatus: "skipped"`.
