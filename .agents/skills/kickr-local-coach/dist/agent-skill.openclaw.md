...
The KICKR app calls the mapped OpenClaw hook `POST /hooks/kickr` with `Authorization: Bearer <OPENCLAW_HOOKS_TOKEN>`. Body:
...
- You are doing the one-time OpenClaw wiring (`OPENCLAW_HOOKS_URL` etc.) — see `references/openclaw-hooks.md` in the KICKR repo.
